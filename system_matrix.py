...
def system_matrix_creation():
    ...
    num_splits = 30
    split_matrices_forward = np.array_split(system_matrices, num_splits)
    split_matrices_back = np.array_split(system_matrices_back, num_splits)
    # Save each submatrix as a separate .pkl file
    for idx, submatrix in enumerate(split_matrices_forward):
        with open(f'/system_matrix/sub_sparse_matrices_forward_{idx}.pkl', 'wb') as f:
            pickle.dump(submatrix, f)
    for idx, submatrix in enumerate(split_matrices_back):
        with open(f'/system_matrix/sub_sparse_matrices_back_{idx}.pkl', 'wb') as f:
            pickle.dump(submatrix, f)
